<template>
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h2 class="text-2xl font-bold text-brand-text">{{ $t('igbh.weekly_title') }}</h2>
        <p class="text-sm text-brand-desc">{{ $t('igbh.weekly_desc') }}</p>
      </div>
      ...
      <div class="flex items-center gap-4 flex-wrap md:flex-nowrap">
        ...
        >
          <span class="text-base font-bold">+</span> {{ $t('igbh.add_weekly') }}
        </button>
      </div>
    </div>
    ...
    <div class="bg-brand-card/40 border border-brand-border p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center justify-between">
      <div class="relative w-full md:w-80">
        <input 
          ...
          v-model="search" 
          @input="fetchData"
          :placeholder="$t('igbh.search_class')" 
      ...
      <table class="w-full text-left border-collapse whitespace-nowrap min-w-max">
        ...
        <tbody class="divide-y divide-brand-border text-sm text-brand-text/90">
          <tr v-for="(item, index) in results" :key="item.id" class="group hover:bg-brand-card transition">
            ...
            <td class="px-6 py-4">{{ item.levelCd || '-' }}</td>
            <td class="px-6 py-4">{{ item.teacherNm || '-' }}</td>
            <td class="px-6 py-4 font-medium text-indigo-400">{{ item.classNm }}</td>
            <td class="px-6 py-4 text-center font-bold">
              <span v-if="item.graded_cnt === item.total_cnt && item.total_cnt > 0" class="text-emerald-500">{{ item.graded_cnt }}/{{ item.total_cnt }}</span>
              <span v-else-if="item.total_cnt > 0" class="text-amber-500">{{ item.graded_cnt }}/{{ item.total_cnt }}</span>
              <span v-else class="text-brand-desc">{{ item.graded_cnt }}/0</span>
            </td>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div class="bg-brand-card border border-brand-border w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div class="px-6 py-4 border-b border-brand-border bg-brand-header flex justify-between items-center">
          <h3 class="text-lg font-bold text-brand-text">{{ $t('igbh.modal.add_weekly_title') }}</h3>
