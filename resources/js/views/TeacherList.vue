<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-white">Teachers</h2>
        <p class="text-sm text-gray-400">Manage instructor accounts and center assignments</p>
      </div>
    ...
    <div class="bg-[#0d1527]/30 border border-gray-800 p-4 rounded-xl flex items-center justify-between">
      <input type="text" v-model="search" placeholder="Search teachers..." class="px-4 py-2 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition duration-150 text-sm w-72">
    </div>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-gray-800 text-sm text-gray-300">
          <tr v-for="teacher in filteredTeachers" :key="teacher.id" class="hover:bg-gray-800/20 transition duration-150">
            <td class="px-6 py-4 font-medium text-white">{{ teacher.ins_name }}</td>
            ...
            <td class="px-6 py-4">
              <span :class="teacher.head === 'Y' ? 'text-indigo-400 bg-indigo-500/10' : 'text-gray-400 bg-gray-500/10'" class="px-2 py-0.5 rounded text-xs font-semibold">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-[#0d1527] border border-gray-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4">
        <h3 class="text-lg font-bold text-white">{{ editingId ? 'Edit Teacher' : 'Add New Teacher' }}</h3>

        <form @submit.prevent="saveTeacher" class="space-y-4">
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Teacher Name</label>
            <input type="text" v-model="form.ins_name" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
          </div>
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Branch LMS ID</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Phone</label>
              <input type="text" v-model="form.phone" class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Head Teacher</label>
              <select v-model="form.head" class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
