<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-white">Contracts & Enrollments</h2>
        <p class="text-sm text-gray-400">Manage student contracts, dates and status codes</p>
      </div>
    ...
    <div class="bg-[#0d1527]/30 border border-gray-800 p-4 rounded-xl flex items-center justify-between">
      <input type="text" v-model="search" placeholder="Search contracts..." class="px-4 py-2 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition duration-150 text-sm w-72">
    </div>
    ...
    <div class="overflow-x-auto bg-[#0d1527]/20 border border-gray-800 rounded-xl">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-gray-800 text-sm text-gray-300">
          <tr v-for="contract in filteredContracts" :key="contract.id" class="hover:bg-gray-800/20 transition duration-150">
            <td class="px-6 py-4 font-medium text-white">{{ contract.student_name }}</td>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-[#0d1527] border border-gray-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4">
        <h3 class="text-lg font-bold text-white">{{ editingId ? 'Edit Contract' : 'Add New Contract' }}</h3>

        <form @submit.prevent="saveContract" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Student Name</label>
              <input type="text" v-model="form.student_name" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Class Name</label>
              <input type="text" v-model="form.class_name" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
            </div>
          </div>
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Branch Name</label>
            <input type="text" v-model="form.branch_name" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Start Date</label>
              <input type="date" v-model="form.enrolment_start_date" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">End Date</label>
              <input type="date" v-model="form.enrolment_last_date" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Valid CD</label>
              <select v-model="form.valid_cd" class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Remark</label>
            <textarea v-model="form.remark" rows="2" class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm"></textarea>
          </div>
...
export default {
  data() {
    return {
      contracts: [
        { id: 1, student_name: 'Nguyễn Văn A', class_name: 'U-Crea PXL K01', branch_name: 'CMS Phan Xích Long', enrolment_start_date: '2026-06-01', enrolment_last_date: '2026-12-01', valid_cd: 'VC005', status: 'SS001', remark: 'Registered through LMS sync' }
      ],
      search: '',
      showModal: false,
      editingId: null,
