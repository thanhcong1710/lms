  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-400">View and edit LMS student sync records</p>
      </div>
    ...
    <div class="bg-[#0d1527]/30 border border-gray-800 p-4 rounded-xl flex items-center justify-between">
      <input type="text" v-model="search" placeholder="Search students..." class="px-4 py-2 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition duration-150 text-sm w-72">
    </div>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-gray-800 text-sm text-gray-300">
          <tr v-for="student in filteredStudents" :key="student.id" class="hover:bg-gray-800/20 transition duration-150">
            ...
            <td class="px-6 py-4 font-mono text-xs text-gray-400">{{ student.accounting_id }}</td>
            <td class="px-6 py-4">{{ student.date_of_birth }}</td>
            <td class="px-6 py-4">
              <span class="text-xs px-2 py-0.5 rounded font-semibold bg-gray-800 text-gray-300">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-[#0d1527] border border-gray-800 rounded-2xl w-full max-w-lg p-6 shadow-2xl space-y-4">
        <h3 class="text-lg font-bold text-white">{{ editingId ? 'Edit Student' : 'Add New Student' }}</h3>
        ...
        <form @submit.prevent="saveStudent" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Accounting ID</label>
              <input type="text" v-model="form.accounting_id" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 text-sm">
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Date of Birth</label>
              <input type="date" v-model="form.date_of_birth" required class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
            </div>
            <div>
              <label class="block text-xs font-semibold text-gray-400 uppercase mb-2">Gender</label>
              <select v-model="form.gender" class="w-full px-4 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-white focus:outline-none focus:border-indigo-500 text-sm">
